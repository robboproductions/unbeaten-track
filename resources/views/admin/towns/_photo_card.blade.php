@php
    ...
    /** @var \App\Models\TownPhoto $photo */
    ...
    $imgAlt = $captionPlain !== '' ? Str::limit($captionPlain, 160) : 'Town photo';
@endphp
...
@if ($layout === 'hero')
    <article class="town-photo-card town-photo-card--{{ $layout }}">
        ...
        <form method="post" action="{{ route('admin.towns.photos.update', [$town, $photo]) }}" class="town-photo-meta-form">
            @csrf
            @method('patch')
            <div class="town-photo-field">
                <label class="town-photo-field-label" for="photo-caption-{{ $photo->id }}">Caption</label>
                <textarea
            ...
            <div class="town-photo-field">
                <label class="town-photo-field-label" for="photo-source-{{ $photo->id }}">Source</label>
                <input
        ...
        <div class="town-photo-card-actions">
            @if (! $photo->is_primary)
                <form method="post" action="{{ route('admin.towns.photos.primary', [$town, $photo]) }}">
                    @csrf
                    <input class="btn btn-neutral btn-sm" type="submit" value="Set as main" />
                </form>
            @endif
            <form method="post" action="{{ route('admin.towns.photos.destroy', [$town, $photo]) }}" onsubmit="return confirm('Remove this photo? This cannot be undone.');">
                @csrf
                @method('delete')
...
@else
    ...
    >
        <button
            ...
            aria-controls="town-photo-dialog-{{ $photo->id }}"
        >
        ...
        <template x-teleport="body">
            <div
                ...
                id="town-photo-dialog-{{ $photo->id }}"
                role="dialog"
                aria-modal="true"
                aria-labelledby="town-photo-dialog-title-{{ $photo->id }}"
            >
                ...
                <div class="town-photo-modal-dialog">
                    <div class="town-photo-modal-head">
                        <h2 class="town-photo-modal-title" id="town-photo-dialog-title-{{ $photo->id }}">Edit photo</h2>
