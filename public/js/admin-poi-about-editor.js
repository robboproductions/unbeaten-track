/**
 * TinyMCE for POI "About" + Draft with Claude.
 * Expects window.__UT_POI_ABOUT_EDITOR before this file loads.
 */
(function () {
    'use strict';

    function cfg() {
        return window.__UT_POI_ABOUT_EDITOR || {};
    }
    ...
    function setAiMessage(text, isError) {
        var el = document.getElementById('poi_about_ai_message');
        if (!el) {
            return;
        }
        if (!text) {
    ...
    function bootAi() {
        var btn = document.getElementById('poi_about_ai_btn');
        var ai = aiCfg();
        if (!btn || !ai.url) {
            return;
        }
            ...
            fetch(ai.url, {
                ...
                .then(function (out) {
                    if (out.ok && out.data && typeof out.data.html === 'string') {
                        var ed = typeof tinymce !== 'undefined' ? tinymce.get('poi_about_html') : null;
                        if (ed) {
                            ed.setContent(out.data.html);
                        } else {
                            var ta = document.getElementById('poi_about_html');
                            if (ta) {
    ...
    if (document.getElementById('poi_about_html')) {
        bootAi();
        bootEditor();
    }
})();
