...
class TownNarrationService
{
    ...
    public function isStale(Town $town): bool
    {
        if (! $town->has_narration) {
            return false;
        }

        return $town->narration_script_hash !== $this->currentHash($town);
    }

    public function currentHash(Town $town): string
    {
        $voiceId = $this->resolveVoiceId($town);
        $modelId = $this->resolveModelId($town);
        $script = (string) ($town->narration_script ?? '');

        return hash('sha256', $script.'|'.$voiceId.'|'.$modelId);
    }

    public function generate(Town $town, User $generatedBy): Town
    {
        if (! config('poi_narration.enabled')) {
            throw new NarrationGenerationException('Narration is disabled in config.');
        }

        $script = trim((string) ($town->narration_script ?? ''));
        if ($script === '') {
        ...
        $dir = (string) config('poi_narration.town_storage.directory', 'town-narrations');
        $filename = $dir.'/'.$town->id.'-'.mb_substr($hash, 0, 12).'.mp3';

        $disk = Storage::disk((string) config('poi_narration.town_storage.disk', 'public'));
        $previousPath = $town->narration_audio_path;
    ...
    public function deleteAudio(Town $town): void
    {
    ...
    private function resolveVoiceId(Town $town): string
    {
        $id = trim((string) ($town->narration_voice_id ?? ''));
        if ($id !== '') {
            return $id;
        }
    ...
    private function resolveModelId(Town $town): string
    {
        $id = trim((string) ($town->narration_model_id ?? ''));
        if ($id !== '') {
            return $id;
        }
