...
/**
 * Reads starter POI .xlsx (README + sheet with "POI" in the name, columns A–K).
 */
final class PoiStarterWorkbookReader
{
    ...
    public function readDataRows(string $absolutePath): array
    {
        ...
        try {
            ...
            $sheetPath = $this->resolvePoiSheetPath($workbookXml, $relsXml);
    ...
    private function resolvePoiSheetPath(string $workbookXml, string $relsXml): string
    {
        ...
        throw new RuntimeException('No worksheet with "POI" in its name was found.');
    }
    ...
    private function parseSheetRows(string $sheetXml, array $sharedStrings): array
    {
        ...
        foreach ($rows as $rowEl) {
            ...
            $poiName = isset($assoc['poi_name']) ? trim((string) $assoc['poi_name']) : '';
            if ($poiName === '') {
                continue;
            }
